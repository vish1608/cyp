const MoreOptions = () => {
    ...
    cy.get(':nth-child(5) > .MuiPaper-root').should('contain', 'अनुवाद')

    cy.get('.MuiSelect-root').select('English')
    cy.get(':nth-child(5) > .MuiPaper-root').should('contain', 'Translate')
    ...
    theme.forEach ( ( selector ) => {
        cy.get('.MuiSelect-root').select(`${selector.a}`)
        cy.get('.MuiButton-label').click()
        cy.get('[style="position: fixed; z-index: 1300; inset: 0px;"] > .MuiPaper-root > .MuiList-root > [tabindex="-1"]').click()
        cy.get(':nth-child(5) > .MuiPaper-root').should('have.css', 'background-color', `rgb(${selector.b})`)
    })
